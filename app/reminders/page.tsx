...
export default function RemindersPage() {
  ...
  async function load() {
    ...
    try {
      const [reminderResponse, obligationResponse] = await Promise.all([fetch('/api/reminders'), fetch('/api/obligations')])
      const reminderData = await reminderResponse.json()
      const obligationData = await obligationResponse.json()
      if (!reminderResponse.ok) throw new Error(reminderData.error || 'Unable to load reminders')
      if (!obligationResponse.ok) throw new Error(obligationData.error || 'Unable to load obligations')
