...
type ObligationsResponse = { obligations?: Item[] }
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    let active = true
    getJson<ObligationsResponse | Item[]>('/api/obligations')
      .then(data => { if (active) setItems(Array.isArray(data) ? data : data.obligations ?? []) })
      .catch(err => { if (active) setError(err instanceof Error ? err.message : 'Unable to load renewals') })
