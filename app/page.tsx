    ...
    <main className="min-h-screen bg-[#f7f8fa] text-slate-950 md:flex">
      ...
      <aside className="hidden min-h-screen w-[252px] shrink-0 flex-col border-r border-slate-200 bg-white px-4 py-5 md:flex">
        <div className="mb-8 flex items-center gap-3 px-2">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-[#174ea6] font-bold text-white">L</div>
          <div><p className="text-lg font-bold tracking-tight">LexRenew</p><p className="text-[11px] text-slate-400">Renewal intelligence</p></div>
        </div>
        ...
        <div className="mt-auto rounded-2xl border border-slate-100 bg-slate-50 p-4">
          <ShieldCheck size={19} className="text-[#174ea6]"/>
      ...
      <section className="min-w-0 flex-1">
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/95 px-5 backdrop-blur md:px-8">
          ...
          {guest ? (
            <div className="flex items-center gap-2">
              ...
              <Link href="/login?mode=signup" className="flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white shadow-sm"><LogIn size={16}/>Get started</Link>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <button onClick={() => setModal(true)} className="hidden items-center gap-2 rounded-xl bg-[#174ea6] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#123f87] sm:flex"><Plus size={16}/>Add obligation</button>
              <ProfileMenu />
            </div>
          )}
        </header>
        ...
        <div className="mx-auto max-w-7xl p-5 md:p-8 lg:p-10">
          {guest && (
            <div className="mb-6 flex flex-col justify-between gap-4 rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 sm:flex-row sm:items-center">
              <div><p className="font-semibold text-[#174ea6]">Demo workspace</p><p className="mt-1 text-sm text-slate-600">Explore the workflow with sample data, then sign in to create your own workspace.</p></div>
              <Link href="/login?mode=signup" className="shrink-0 rounded-xl bg-[#174ea6] px-4 py-2.5 text-sm font-semibold text-white">Start using LexRenew</Link>
            </div>
          )}
          ...
          <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            {!guest && <button onClick={() => setModal(true)} className="flex items-center justify-center gap-2 rounded-xl bg-[#174ea6] px-4 py-2.5 text-sm font-semibold text-white shadow-sm sm:hidden"><Plus size={16}/>Add obligation</button>}
          </div>
          ...
          <div className="mb-6 flex gap-3 rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
            <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-[#174ea6]"><Sparkles size={18}/></div>
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col justify-between gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center">
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <div className="relative"><Search size={16} className="absolute left-3 top-3 text-slate-400"/><input value={q} onChange={event => setQ(event.target.value)} placeholder="Search obligations" className="w-full rounded-xl border border-slate-200 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[#174ea6] sm:w-56"/></div>
                <select value={filter} onChange={event => setFilter(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#174ea6]"><option>All</option>{types.map(type => <option key={type}>{type}</option>)}</select>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-5 backdrop-blur-sm">
          <form onSubmit={add} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              <input name="title" required placeholder="Document / obligation name" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-[#174ea6]"/>
              <select name="type" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-[#174ea6]">{types.map(type => <option key={type}>{type}</option>)}</select>
              <input name="counterparty" placeholder="Counterparty / authority" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-[#174ea6]"/>
              <input name="expiry" required type="date" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:border-[#174ea6]"/>
            </div>
            <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setModal(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium">Cancel</button><button disabled={saving} className="rounded-xl bg-[#174ea6] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Saving…' : 'Save obligation'}</button></div>
          </form>
        </div>
      )}
    </main>
...
function Nav({ href, icon, label, active = false }: { href: string; icon: React.ReactNode; label: string; active?: boolean }) {
  return <Link href={href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 font-medium transition ${active ? 'bg-slate-100 text-slate-950' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`}>{icon}<span>{label}</span></Link>
}

function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div className="grid h-9 w-9 place-items-center rounded-xl bg-blue-50 text-[#174ea6]">{icon}</div><span className="text-[11px] font-medium uppercase tracking-[0.14em] text-slate-300">Live</span></div><p className="mt-5 text-sm text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold tracking-tight">{value}</p></div>
}
